    ...
    <section className="py-15 Popular-bg">
      <div className="containers">
          ...
          {dishes.map((item) => (
            <li key={item} className="relative transition-transform duration-300 hover:scale-110 cursor-pointer">
              <div className="hero-bg rounded-[38px] pt-40 p-7.5">
                ...
                <p className="text-[15px] text-[#000000] mb-1">
                  {t("dishDesc")}
                </p>
                <div className="flex items-end justify-between">
                  ...
                  <Button 
                    type="button" 
                    extraStyle="p-[12px] bg-[#000000] rounded-[5px] hover:bg-green-600 transition-colors" 
                    icon={<PapularShopIcon />} 
                    iconPost="left" 
                  />
                </div>
              </div>
        ...
        <div className="flex justify-end">
          <Button 
            type="button" 
            title={tHero("button")} // Boyagi Hero'dagi "Посмотреть меню" ni ishlatamiz
            icon={<IconArrowRight />} 
            iconPost="right" 
